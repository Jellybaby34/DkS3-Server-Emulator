...
public:
	...
	// Use if you want to push work to the pool without needing the return value
	template<typename F, typename... Args>
	void enqueue_work(F&& f, Args&&... args)
	{
		using value_type = typename std::decay<F>::type;
		auto work = [p = std::forward<value_type>(f), t = std::make_tuple(std::forward<Args>(args)...)]() { std::apply(p, t); };
		auto i = m_index++;
	...
	// Use if you want the return value from the function you're pushing to the pool
	template<typename F, typename... Args>
